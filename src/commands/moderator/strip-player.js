...
module.exports = new ChatInputCommand({
  ...
  global: true,
  data: {
    description: 'Strip a player that is currently online, removing everything from their inventory',
    options: [
      requiredServerConfigCommandOption,
      requiredPlayerSessionOption,
      {
  ...
  run: async (client, interaction) => {
    ...
    if (res !== true) {
      interaction.editReply({ content: `${ emojis.error } ${ member }, invalid response code - **\`${ session.playerName }\`** might not have been killed by stripped` });
      return;
    }

    // Notify player
    if (notifyPlayer) {
      await messageSurvivor(serverCfg.CFTOOLS_SERVER_API_ID, session.id, 'You have been stripped of all your possessions by an administrator');
    }

    // User feedback on success
    interaction.editReply({ content: `${ emojis.success } ${ member }, **\`${ session.playerName }\`** has been stripped of all their possessions!` });
  }
});
