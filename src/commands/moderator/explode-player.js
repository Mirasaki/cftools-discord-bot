...
module.exports = new ChatInputCommand({
  ...
  global: true,
  data: {
    description: 'Explode a player that is currently online',
    options: [
      requiredServerConfigCommandOption,
      requiredPlayerSessionOption,
      {
  ...
  run: async (client, interaction) => {
    ...
    if (res !== true) {
      interaction.editReply({ content: `${ emojis.error } ${ member }, invalid response code - **\`${ session.playerName }\`** might not have been killed by explosion` });
      return;
    }

    // Notify player
    if (notifyPlayer) {
      await messageSurvivor(serverCfg.CFTOOLS_SERVER_API_ID, session.id, 'You have been killed by an explosion, as executed by an administrator');
    }

    // User feedback on success
    interaction.editReply({ content: `${ emojis.success } ${ member }, **\`${ session.playerName }\`** has been killed by an explosion!` });
  }
});
